...
def get_gspread_client() -> gspread.Client:
    ...
    # scopes recomendados (si tu service account ya funciona, esto igual funciona)
    scopes = [
...
def load_tenants(gc: gspread.Client, force: bool = False) -> Dict[str, Dict[str, Any]]:
    ...
    rows = ws.get_all_records()  # asume headers técnicos fila 1
    ...
    for r in rows:
        ...
        tenants[tid] = {
            ...
            # nuevos nombres (los que tú ya tienes)
...
def send_admin_order_message(tenant: Dict[str, Any], order_id: str, items: List[Dict[str, Any]], total_amount: float,
                             customer_name: str, customer_contact: str, notes: str, delivery_type: str, requested_time: str) -> None:
    bot_token = tenant.get("admin_bot_token", "")
    admin_chat_id = tenant.get("admin_chat_id", "")
    if not bot_token or not admin_chat_id:
        log_event("admin_notify_skip", tenant_id=tenant.get("tenant_id"), has_token=bool(bot_token), has_admin=bool(admin_chat_id))
        return
...
async def telegram_client_webhook(tenant_id: str, secret: str, update: Dict[str, Any]):
    ...
    bot_token = tenant.get("client_bot_token", "").strip()
    if not bot_token:
    ...
    if callback:
        ...
        # Acknowledge rápido para que Telegram no “cargue”
        ...
        if data == "confirm":
            ...
            # Para demo: nombre/contacto “Telegram”
            ...
            try:
                ...
                log_event("admin_notify_error", tenant_id=tenant_id, order_id=order_id, error=str(e))
...
async def telegram_admin_webhook(tenant_id: str, secret: str, update: Dict[str, Any]):
    ...
    bot_token = tenant.get("admin_bot_token", "").strip()
    ...
    if not result.get("found"):
        ...
        # Igual contestamos callback para que Telegram no se quede cargando
        if bot_token:
            telegram_api_call(bot_token, "answerCallbackQuery", {
